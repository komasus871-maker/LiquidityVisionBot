...
from services.pump_dump_scanner import ScannerRepository, Severity, resource_budget
...
async def pump_scan(message: Message) -> None:
    rows = repository.recent(12, telegram_id=message.from_user.id)
    stats = repository.stats_24h(telegram_id=message.from_user.id)
    lines = ["⚡ <b>RECENT MARKET ALERTS</b>", "<i>Not trade signals</i>", ""]
    if not rows:
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{row['window_minutes']}m · {escape(str(row['severity']))}"
        )
