...
async def admin_status(message: Message) -> None:
    ...
    await message.answer(
        "\n".join([
            ...
            f"LIVE read-only/certified/enabled/suspended: {counts.get('live_read_only_accounts', 0)}/{counts.get('live_certified_accounts', 0)}/{counts.get('live_enabled_accounts', 0)}/{counts.get('live_suspended_accounts', 0)}",
            f"LIVE queue planned/claimed/recovery: {counts.get('live_queue_planned', 0)}/{counts.get('live_queue_claimed', 0)}/{counts.get('live_queue_recovery', 0)}",
            f"LIVE reconciliation/PnL failures/kill switches: {counts.get('live_reconciliation_unresolved', 0)}/{counts.get('live_daily_pnl_failures', 0)}/{counts.get('live_active_kill_switches', 0)}",
            "",
