...
class TradeIntelligenceEngine:
    """Deterministic live-trade intelligence with a single canonical snapshot."""
    ...
    @staticmethod
    def _previous_snapshot(signal: dict) -> dict[str, Any]:
        raw = signal.get("intelligence_json") or "{}"
        if isinstance(raw, dict):
            return raw
        try:
            return json.loads(raw)
    ...
    def evaluate(self, signal: dict, price: float, df=None) -> IntelligenceSnapshot:
        ...
        previous = self._previous_snapshot(signal)
        component_deltas = {
            key: round(float(value) - float(previous.get(key, value) or value), 1)
        ...
        previous_confidence = float(previous.get("confidence", signal.get("dynamic_confidence") or signal.get("confidence") or confidence))
        ...
        for key, delta in sorted(component_deltas.items(), key=lambda item: abs(item[1]), reverse=True):
            if abs(delta) >= 6:
                reasons.append(f"{key.capitalize()} {'improved' if delta > 0 else 'weakened'} {delta:+.0f}")
        previous_risk = float(previous.get("risk_used", signal.get("last_risk_used") or 0))
        for level in (75, 90):
            if previous_risk < level <= risk_used:
                reasons.append(f"Risk used crossed {level}%")
        previous_giveback = float(previous.get("mfe_giveback", signal.get("last_mfe_giveback") or 0))
        ...
        if confidence_delta <= -6:
            sentences.append(f"Confidence decreased by {abs(confidence_delta):.0f} points during the latest update.")
        elif confidence_delta >= 6:
            sentences.append(f"Confidence improved by {confidence_delta:.0f} points during the latest update.")
        if risk_used >= 75:
        ...
        sentences.append("Price remains on the favorable side of the entry." if current_r > 0 else "Price is currently trading against the entry.")
