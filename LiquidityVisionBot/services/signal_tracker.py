...
class SignalTracker:
    ...
    async def _transition(self, signal: dict, event: str, price: float, **fields) -> None:
        ...
        if event in {"TP3", "STOP", "BREAKEVEN", "INVALIDATED", "EXPIRED"}:
            self.history.save_learning_sample(signal["id"])
    ...
    async def _update(self, signal: dict, price: float, df) -> None:
        ...
        self.history.save_intelligence_snapshot(signal["id"], intelligence_payload)
        intelligence_fields = {
