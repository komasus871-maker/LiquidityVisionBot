...
async def journal_handler(message: Message):
    ...
    for item in recent:
        ...
        if item["status"] in {"ACTIVE", "TP1", "TP2"}:
            snapshot = history.get_latest_intelligence_snapshot(int(item["id"])) or {}
            health = html.escape(str(snapshot.get("health") or item.get("trade_health") or "🟡 STABLE"))
            health_score = float(snapshot.get("health_score") or item.get("health_score") or 0)
            confidence = float(snapshot.get("confidence") or item.get("dynamic_confidence") or item.get("confidence") or 0)
            intelligence = f"\n   {health_score:.0f}% · {health} · Confidence {confidence:.0f}%"
        recent_text.append(
...
async def trade_replay_handler(message: Message):
    ...
    snapshot = history.get_latest_intelligence_snapshot(signal_id) or {}
    intelligence_timeline = history.get_intelligence_timeline(signal_id, limit=8)
    if intelligence_timeline:
        timeline_lines = []
        for item in intelligence_timeline:
            created = str(item.get("created_at") or "")
            try:
                stamp = datetime.fromisoformat(created).strftime("%d.%m %H:%M")
            except ValueError:
                stamp = created[:16]
            reasons = _event_details(item.get("reasons_json"))
            reason_text = f" · {html.escape(str(reasons[0]))}" if reasons else ""
            timeline_lines.append(
                f"• {stamp} · Confidence {float(item.get('confidence') or 0):.0f}% "
                f"· Health {float(item.get('health_score') or 0):.0f}% {html.escape(str(item.get('health') or ''))}{reason_text}"
            )
        intelligence_replay = "\n".join(timeline_lines)
    else:
        intelligence_replay = "Intelligence history is still collecting."
...
Trade Health: <b>{float(snapshot.get('health_score') or signal.get('health_score') or 0):.0f}% · {html.escape(str(snapshot.get('health') or signal.get('trade_health') or '—'))}</b>
Dynamic Confidence: <b>{float(snapshot.get('confidence') or signal.get('dynamic_confidence') or signal.get('confidence') or 0):.0f}%</b>
