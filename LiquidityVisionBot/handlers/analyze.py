...
async def watchlist_view(message: Message):
    ...
    for index, row in enumerate(rows, 1):
        ...
        checked = row.get("last_checked_at") or row.get("updated_at")
        error = row.get("last_error")
        lines.append(f"{index}. <b>{row['symbol']}</b> · {row['timeframe'].upper()}")
        if not status:
            lines.append("   ⏳ Первый анализ ещё выполняется…")
        else:
            lines.append(f"   {status} · Dir {direction:.1f} · Ready {readiness:.1f}")
            lines.append(f"   Проверено: <code>{checked or 'ещё не проверено'}</code>")
        if error:
