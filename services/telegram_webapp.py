...
function draw(data){cards.innerHTML="";if(data.available_views){const chooser=document.createElement("div");chooser.className="card";data.available_views.forEach(mode=>{const button=document.createElement("button");button.textContent=mode.replace(/_/g," ");button.classList.toggle("active",mode===data.selected_view);button.onclick=()=>load("scanner?view="+encodeURIComponent(mode));chooser.appendChild(button)});cards.appendChild(chooser)}const rows=data.items||[data];if(!rows.length){const empty=document.createElement("div");empty.className="card muted";empty.textContent="No current records available for this view.";cards.appendChild(empty)}rows.forEach(x=>{const d=document.createElement("div");d.className="card";d.textContent=Object.entries(x).filter(([k])=>!k.endsWith("_json")).map(([k,v])=>k.replace(/_/g," ")+": "+clean(typeof v==="object"?JSON.stringify(v):v)).join("\n");cards.appendChild(d)});status.className="muted";status.textContent="Terminal loaded · "+(data.classification||"current state");setActions(false)}
